...
# satoshi per kbyte
# Doichain's DEFAULT_MIN_RELAY_TX_FEE in validation.h is 100x that of Bitcoin.
...
# Regtest feerate doesn't need Doichain's 100x adjustment.
...
class SimpleConfig(Logger):
    ...
    def electrum_path(self):
        ...
        self.logger.info(f"electrum-doi directory {path}")
        return path
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'doi':8, 'mdoi':5, 'udoi':2, 'bits':2, 'swartz':0}
    ...
    def get_wallet_path(self, *, use_gui_last_wallet=False):
        ...
        old_path = os.path.join(self.path, "electrum-doi.dat")
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-doi.conf into user_config[]."""
    if not path:
        return {}
