...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_doi."):
...
electrum_logger = logging.getLogger("electrum_doi")
electrum_logger.setLevel(logging.DEBUG)


def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_doi_log_*.log")), reverse=True)
    for f in files[keep:]:
        os.remove(str(f))
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_doi_log_{timestamp}_{PID}.log"
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum."):
        name = name[9:]
    if name.startswith("electrum_doi."):
        name = name[13:]
...
def configure_logging(config):
    ...
    _logger.info(f"Electrum-DOI version: {ELECTRUM_VERSION} - https://www.namecoin.org - {GIT_REPO_URL}")
