AppImage binary for Electrum-DOI
============================
...
2. Build image
    ...
    ```
    $ sudo docker build -t electrum-doi-appimage-builder-img contrib/build-linux/appimage
    ```
...
3. Build binary
    ...
    ```
    $ sudo docker run -it \
        --name electrum-doi-appimage-builder-cont \
        -v $PWD:/opt/electrum-doi \
        --rm \
        --workdir /opt/electrum-doi/contrib/build-linux/appimage \
        electrum-doi-appimage-builder-img \
        ./build.sh
    ```
...
Execute the binary as follows: `./electrum-doi*.AppImage --appimage-extract`

### How to investigate diff between binaries if reproducibility fails?
```
cd dist/
./electrum-doi-*-x86_64.AppImage1 --appimage-extract
mv squashfs-root/ squashfs-root1/
./electrum-doi-*-x86_64.AppImage2 --appimage-extract
